...
//		testEncrypt();
	...
	private static void testSubscribe() {
		Chainsql.event.subscribeTable(sTableName, rootAddress, (data) -> {
...
//		test.testOperationRule();
	...
	private static void testAccount() {
		...
		String str = c.getAccountBalance(sNewAccountId);
...
//		c.beginTran();
//		List<String> args =
//		Util.array("{'field':'id','type':int,'length':111}","{'field':'name','type':'varchar','length':50,'default':'abc'}","{'field':'age','type':'int'}");
//		c.createTable(sTableName,args,false);
//		List<String> parm1 = Util.array("('$IsExisted':1}");
//		c.table(sTableName).sqlAssert(parm1);
//		c.table(sTableName).insert(Util.array("{'id':1 }","{'id':2,'name':'cd'}","{'id':3,'name':'cd'}","{'id':4,'name':'ef'}"));
//		List<String>parm2 = Util.array("{'$RowCount':1}","{'id':1,'name':'ac'}");
...
//	}
	...
	public void getTransaction() {
		String hash = "4FA308EF36BADDBD5343D05FF353D95E04B6241F785920E5689600642B494967";
...
//				});
	...
	public void activateAccount(String account) {
		JSONObject ret = c.pay(account, "20000");
